...
class Blockchain {
  ...
  requestMessageOwnershipVerification(address) {
    return new Promise((resolve) => {
      resolve(`${address}:${new Date().getTime().toString().slice(0,
          -3)}:starRegistry`);
    });
  }
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (currentTime - msgTime < (300)) {
        reject("ERROR - Time elapsed must less than 5 minutes");
      }
      ...
      if (isValid) {
        try {
          const newBlock = await self._addBlock(
              new BlockClass.Block({addr: address, star: star}));
          resolve(newBlock);
        } catch (err) {
          reject(err);
        }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      let retVal = self.chain.find(block => block.hash === hash);
      if (retVal) {
        resolve(retVal);
      } else {
        resolve(null);
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise((resolve, reject) => {
      self.chain.filter((block) => {
        ...
        if (blockData && blockData.addr === address) {
