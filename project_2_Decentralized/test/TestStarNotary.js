...
it('lets user2 buy a star and decreases its balance in ether', async () => {
  ...
  let value = Number(balanceOfUser2BeforeTransaction) - Number(
...
it('can add the star name and star symbol properly', async () => {
  ...
  let tokenId = 6;
  await instance.createStar('VOI', tokenId, {from: accounts[0]});
  // 2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(await instance.name.call(), "Kien Token");
  assert.equal(await instance.symbol.call(), "KNT");
});
...
it('lets 2 users exchange stars', async () => {
  ...
  let instance = await StarNotary.deployed();
  let tokenId1 = 7;
  await instance.createStar('VOI', tokenId1, {from: accounts[0]});

  let tokenId2 = 8;
  await instance.createStar('VOI', tokenId2, {from: accounts[1]});
